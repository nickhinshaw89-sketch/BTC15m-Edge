...
def decision_from_live(cfg, market, kstate, bstate, store):
    ...
    if not any(best.get(k) is not None for k in ("yes_best_bid_c", "yes_best_ask_c", "no_best_bid_c", "no_best_ask_c")):
        ...
        if fb:
            best = fb
    direct, _reverse = construct_direct_and_reverse_candidates(
    ...
    decision = evaluate_selected_direct_candidate(
        direct,
        already_traded_market=store.already_traded(market.ticker),
    )
    decision["place_orders"] = int(cfg.place_orders)
    return decision
